...
func main() {
	...
	app := cli.NewApp()
	app.Name = "cisco spark plugin"
	app.Usage = "cisco spark plugin"
	app.Action = run
	app.Version = "0.0.1"
	app.Flags = []cli.Flag{
		// Plugin environment
		cli.StringFlag{
			Name:   "access_token",
			Usage:  "spark bot's access token",
			EnvVar: "PLUGIN_ACCESS_TOKEN,SPARK_ACCESS_TOKEN",
		},
		cli.StringFlag{
			Name:   "room",
		...
		cli.StringFlag{
			Name:   "api_endpoint",
			Usage:  "spark api endpoint",
			EnvVar: "PLUGIN_API_ENDPOINT,SPARK_API_ENDPOINT",
			Value:  DefaultApi,
		},
		cli.StringFlag{
            Name:   "template.body",
